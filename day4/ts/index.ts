...
const part2 = (input: Array<Array<String>>, codes: Array<string>): Number => {
    ...
    input.map((v) => {
      z = 0;
      v.map((k) => {
        for (let code of codes) if (k.startsWith(code)){
            if(code == "byr"){
                if (parseInt(k.split(':')[1]) >= 1920 && parseInt(k.split(':')[1]) <= 2002) z++
            }
            if(code == "iyr"){
                if (parseInt(k.split(':')[1]) >= 2010 && parseInt(k.split(':')[1]) <= 2020) z++
            }
            if(code == "eyr"){
                if (parseInt(k.split(':')[1]) >= 2020 && parseInt(k.split(':')[1]) <= 2030) z++
            }
            if(code == "hgt"){
            ...
            if(code == "pid"){
                if(k.split(":")[1].length === 9 && parseInt(k.split(":")[1]) >= 0 && parseInt(k.split(":")[1]) <= 999999999) z++;
            }
        }
      });
      if (z == 7) s++;
